...
Users is a distinct bounded context because it owns identity profile data, contact fields, avatar metadata, onboarding progress, and active address pointer. Other modules interact with it through typed IDs, commands, queries, and domain events; they do not write its persistence rows directly.

## Domain Model Audit

The current design centers on `User` as the aggregate root and `UserId` as the strongly typed identifier. Domain behavior belongs on the aggregate or on domain services listed in `domain/services`; DTOs, Prisma rows, GraphQL types, and external adapter payloads remain outside the domain model.
...
`IUserRepository` is the application/domain boundary for persistence. The Prisma implementation belongs under `infrastructure/repositories`, and mapping is handled by injectable mapper classes so the domain layer stays persistence-ignorant.
...
model User {
  ...
  @@unique([email])
  @@unique([phoneNumber])
  @@index([activeAddressId])
}
```

Scalar-ID references and cleanup owners:
- `activeAddressId` references Address. Cleanup owner: DeleteUserAddressCommandHandler dispatches ClearActiveAddressCommand before deleting an active address.

Indexes and constraints mapped to repository methods/domain errors:
- `@@unique([email])` maps to `findByEmail`; domain error: `EmailAlreadyInUseError`.
- `@@unique([phoneNumber])` maps to `profile update identity checks`; domain error: `PhoneNumberAlreadyInUseError`.
- `@@index([activeAddressId])` maps to `OnAddressDeletedClearActiveAddressHandler`; domain error: `none`.
...
class User extends AggregateRoot<UserId> {
  ...
  /**
   * Rehydrates `User` from persistence.
...
interface IUserRepository {
   ...
   * 4. Map the row with `UserMapper.toDomain(row)` when present.
   */
  findById(id: UserId): Promise<User | null>;

  /**
   * Persists the aggregate in one durable write boundary.
   * 1. Convert the aggregate with `UserMapper.toPersistence(aggregate)`.
   * 2. Execute the Prisma create/update/upsert for `User`.
 ...
 * - `userRepository: IUserRepository` loads and saves `User` aggregate state.
 ...
 * 2. Load required `User` state through `userRepository` using the command's typed ID fields.
 * 3. // TODO: Confirm the exact `User.updateUserProfile(...)` domain method signature if it is not already declared in the Domain Layer.
 * 4. Persist the aggregate or records through `userRepository.save(aggregate)`.
...
class UpdateUserProfileHandler implements ICommandHandler<UpdateUserProfileCommand> {
  /**
   * Executes `UpdateUserProfileCommand`.
   * 1. Validate `command.payload` and typed IDs.
   * 2. Load `User` state with `userRepository.findById(...)` when the command targets an existing aggregate.
   * 3. // TODO: Confirm exact `User` domain method for `UpdateUserProfile` if not declared above.
   * 4. Persist with `userRepository.save(aggregate)`.
 ...
 * 1. Validate the command payload and caller identity fields.
 * 2. Load required `User` state through `userRepository` using the command's typed ID fields.
 * 3. // TODO: Confirm the exact `User.setActiveAddress(...)` domain method signature if it is not already declared in the Domain Layer.
 * 4. Persist the aggregate or records through `userRepository.save(aggregate)`.
 * 5. Translate repository unique-constraint failures into the module domain error named in Persistence Model.
 * 6. After save succeeds, call `aggregate.pullDomainEvents()`.
 * 7. Publish each event with `eventBus.publish(event)`.
...
class SetActiveAddressHandler implements ICommandHandler<SetActiveAddressCommand> {
  /**
   * Executes `SetActiveAddressCommand`.
   * 1. Validate `command.payload` and typed IDs.
   * 2. Load `User` state with `userRepository.findById(...)` when the command targets an existing aggregate.
   * 3. // TODO: Confirm exact `User` domain method for `SetActiveAddress` if not declared above.
   * 4. Persist with `userRepository.save(aggregate)`.
   * 5. Call `aggregate.pullDomainEvents()` only after save succeeds.
   * 6. Publish events with `eventBus.publish(event)`.
   */
 ...
 * 1. Validate the command payload and caller identity fields.
 * 2. Load required `User` state through `userRepository` using the command's typed ID fields.
 * 3. // TODO: Confirm the exact `User.clearActiveAddress(...)` domain method signature if it is not already declared in the Domain Layer.
 * 4. Persist the aggregate or records through `userRepository.save(aggregate)`.
 * 5. Translate repository unique-constraint failures into the module domain error named in Persistence Model.
 * 6. After save succeeds, call `aggregate.pullDomainEvents()`.
 * 7. Publish each event with `eventBus.publish(event)`.
...
class ClearActiveAddressHandler implements ICommandHandler<ClearActiveAddressCommand> {
  /**
   * Executes `ClearActiveAddressCommand`.
   * 1. Validate `command.payload` and typed IDs.
   * 2. Load `User` state with `userRepository.findById(...)` when the command targets an existing aggregate.
   * 3. // TODO: Confirm exact `User` domain method for `ClearActiveAddress` if not declared above.
   * 4. Persist with `userRepository.save(aggregate)`.
   * 5. Call `aggregate.pullDomainEvents()` only after save succeeds.
   * 6. Publish events with `eventBus.publish(event)`.
   */
 ...
 * Constructor dependencies:
 * - `userRepository: IUserRepository` reads `User` persistence state.
 * - `UserMapper: UserMapper` converts domain/read rows to application DTOs.
 * 1. Validate query filters, pagination, and authenticated caller identity.
 * 2. Read data through `userRepository` or the module read-model repository named in the Persistence Model.
 * 3. Scope the read to the caller IDs carried by the query.
 * 4. Convert domain/read rows to application DTOs with `UserMapper`.
...
class GetUserProfileHandler implements IQueryHandler<GetUserProfileQuery> {
  /**
   * Executes `GetUserProfileQuery`.
   * 1. Validate query filters, pagination, and caller identity.
   * 2. Read via `userRepository` or the module read model.
   * 3. Map rows with `UserMapper`.
 ...
 * `toDomain(row)` converts persistence rows to `User.reconstitute(...)` inputs.
