...
model Rating {
  ...
  @@unique([errandId, raterId]) // backs: DuplicateRatingError
  @@index([raterId, createdAt]) // serves: findByRater
  @@index([rateeId, createdAt]) // serves: findByRatee, calculateAverageRating, getRatingStats
}
...
model RatingReaction {
  ...
  @@index([ratingId]) // serves: Rating aggregate reconstitution
  @@unique([ratingId, userId, emoji]) // backs: DuplicateReactionError
}
```

`Score`, `Emoji`, and reply are inside the Rating aggregate; `RatingReaction` has a small lifecycle but is reachable only through `IRatingRepository`. References are scalar IDs only: `raterId`, `rateeId`, `errandId`, `ratingId`, `userId`. Cleanup owners: `UserDeletedPolicyHandler` anonymizes rating/reaction display data without rewriting history; `ErrandDeletedPolicyHandler` prevents hard deletion of rated errands; `RatingDeletedPolicyHandler` removes reactions only through Rating behavior if deletion is ever allowed. `id` serves `findById`, the rating unique pair serves `findByErrandAndRater`, and the other indexes map to rating repository/read methods.
