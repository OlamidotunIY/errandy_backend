...
| **Rating**          | `IRatingRepository`              | `findById(id: RatingId)`, `findByRater(raterId: UserId)`, `findByRatee(rateeId: UserId)`, `save(rating: Rating)`                                             | Rating aggregate persistence + stats aggregation   |
| **Chat**            | `IChatRoomRepository`            | `findById(id: ChatRoomId)`, `findByErrandAndParticipants(errandId: ErrandId, participantIds: UserId[])`, `findMessages(chatRoomId: ChatRoomId)`              | Chat room aggregate persistence + messaging        |
| **Provider**        | `IProviderRepository`            | `findById(id: ProviderId)`, `findByUserId(userId: UserId)`, `save(provider: Provider)`                                                                       | Provider aggregate persistence + discovery queries |
| **Client**          | `IClientRepository`              | `findById(id: ClientId)`, `findByUserId(userId: UserId)`, `save(client: Client)`                                                                             | Client aggregate persistence                       |
| **Address**         | `IUserAddressRepository`         | `findByUserId(userId: UserId)`, `findById(id: AddressId)`, `save(address: UserAddressSnapshot)`                                                              | User address persistence for geocoding + profiles  |
| **Auth**            | `IAuthRepository`                | `findIdentityByUserId(userId: UserId)`, `findSessionByToken(token: string)`, `saveVerification(...)`                                                         | Auth identity/session lookup + verification store  |
| **Dispute**         | `IDisputeRepository`             | `findById(id: DisputeId)`, `findByErrandId(errandId: ErrandId)`, `findPendingByErrandId(errandId: ErrandId)`                                                 | Dispute lifecycle persistence                      |
| **Organization**    | `IOrganizationRepository`        | `findById(id: OrganizationId)`, `findByOwnerId(ownerId: UserId)`, `findByMemberUserId(userId: UserId)`                                                       | Organization and membership persistence            |
| **Payment-Gateway** | `IPaymentMethodRepository`       | `findById(id: PaymentMethodId)`, `findByUserId(userId: UserId)`, `findDefaultByUserId(userId: UserId)`                                                       | Payment method + provider customer mapping         |
| **Service**         | `IServiceRepository`             | `findServicesByCategoryId(categoryId: ServiceCategoryId)`, `findServiceById(id: ServiceId)`                                                                  | Read model for service catalog                     |
| **Trusted-Circle**  | `ITrustedCircleRepository`       | `findById(id: TrustedCircleId)`, `findByClientId(clientId: ClientId)`, `save(circle: TrustedCircleAggregate)`                                                | Trusted circle and member persistence              |
| **Verification**    | `IVerificationRepository`        | `findById(id: VerificationId)`, `findByProviderAndType(providerId: ProviderId, type: VerificationType)`, `save(verification: ProviderVerificationAggregate)` | Provider verification persistence                  |
