...
class Money {
  private constructor(
    ...
    if (!Number.isInteger(amountMinorUnits)) {
      throw new InvalidAmountError('Money amount must be an integer number of minor units');
    }
  ...
  add(other: Money): Money {
    ...
    return Money.fromMinorUnits(this.amountMinorUnits + other.amountMinorUnits, this.currency);
  }

  subtract(other: Money): Money {
    this.assertSameCurrency(other);
    if (this.amountMinorUnits < other.amountMinorUnits) {
      throw new NegativeAmountError('Subtraction would result in a negative amount');
    }
    return Money.fromMinorUnits(this.amountMinorUnits - other.amountMinorUnits, this.currency);
  }

  multiply(factor: number): Money {
    return Money.fromMinorUnits(Math.round(this.amountMinorUnits * factor), this.currency);
  }
  ...
  split(weights: number[]): Money[] {
    ...
    if (totalWeight <= 0) {
      throw new InvalidAmountError('Split weights must sum to a positive number');
    }

    const exact = weights.map((w) => (this.amountMinorUnits * w) / totalWeight);
    const floors = exact.map(Math.floor);
    const remainder = this.amountMinorUnits - floors.reduce((sum, f) => sum + f, 0);
  ...
  toString(): string {
    ...
    return new Intl.NumberFormat('en', { style: 'currency', currency: this.currency }).format(
      this.toMajorUnits(),
    );
  }
  ...
  static fromJSON(json: { amountMinorUnits: number; currency: string }): Money {
    return Money.fromMinorUnits(json.amountMinorUnits, json.currency as Currency);
  }
}
