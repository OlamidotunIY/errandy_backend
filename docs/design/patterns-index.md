...
| Pattern | Where |
|---|---|
| **Aggregate** | `Application`, `Errand`, `Escrow`, `Wallet`, `Party` (root, with `Person`/`Organization`/`ProviderRole` as internal entities), `VerificationProfile`, `TrustedCircle`, `Dispute`, `Rating` |
| **Aggregate boundary in the schema** | Any reference *within* an aggregate's own internal entities may use a Prisma `@relation` (e.g. `Person`/`Organization`/`ProviderRole` sharing a PK with `Party`) — that's normal composition. Any reference to a **different** aggregate (`Application.applicantId`, `Errand.clientId`, `TrustedCircleMember.memberId`, `Rating.raterId`/`rateeId`) must be a **plain scalar `String`/`ObjectId` field, never a `@relation`** — this is the DDD "aggregates reference each other by ID only" rule enforced at the schema level, not just in prose. |
...
| Nearby-errand discovery for workers | MongoDB `2dsphere` index (created outside Prisma's schema — see `errand-discovery-flow.md`), then composite-score ranking + top-K | Not CLRS territory for the geospatial part. Practical answer: use MongoDB's native geospatial index, with the location denormalized directly onto the queried document (no cheap cross-collection joins in Mongo). Ranking/top-K reuses the heap approach above. |
