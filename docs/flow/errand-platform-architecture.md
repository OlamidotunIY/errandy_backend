...
4. **Multi-currency, market-isolated.** Every `XMinorUnits` field became `Money { amountMinorUnits, currency }`. A new `Market` entity (seed data, not admin-managed yet) ties one country to one currency. Every `Profile` and `Errand` has a `marketId`; a provider in one market structurally cannot see or apply to errands in another (same principle as Uber's city isolation) — no cross-market/cross-currency logic exists anywhere.
