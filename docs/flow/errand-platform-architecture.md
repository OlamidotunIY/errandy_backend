...
15. **Cancel** (`Application.cancel()`, `Errand.cancel()`) remains deferred.
...
See `docs/flows/README.md` for all 11: application accept, user registration, errand discovery, errand creation, errand reassignment, trusted circle, verification, errand completion, withdrawal, dispute, rating.

## Patterns index

See `docs/flows/patterns-index.md` — every EIP/DDD pattern and algorithm callout, cross-referenced to where it's used.

16. **`Wallet` redesigned**: no separate `Withdrawal` entity — `payment-gateway`'s `PaymentTransaction` gained a `direction: CHARGE|TRANSFER` field and owns outbound transfer status instead (avoids duplicating status-tracking across two modules). `Wallet.partyId` (not `userId` — organizations need their own withdrawable wallet). Three-tier balance (`ACTIVE_ERRAND`/`PENDING`/`AVAILABLE`, via paired `LedgerEntry` bucket-transfers) plus sequence-based optimistic concurrency (`WalletBalanceSnapshot.lastSequence`) replace the earlier flat cached-balance design. `BankAccount` matches on `currency`, not `marketId`. See `docs/modules/wallet.md`.
17. **Dispute window = wallet's pending window (7 days, tunable)** — since escrow releases immediately on completion (not waiting for a dispute window itself), the actual late-dispute protection is the wallet's `PENDING` hold. A dispute resolved within that window reverses the wallet's pending balance (`ReversePendingCommand`), not `Escrow.refund()` (escrow is already finalized by then). `Escrow.refund()` now only applies to the deferred pre-completion cancel path. See `docs/modules/dispute.md`.
